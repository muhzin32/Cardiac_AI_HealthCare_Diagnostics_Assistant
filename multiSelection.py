...
try:
    for lead in range(12):
        model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "models", "MultiLabel", "Models", f"model_lead_{lead}.h5")
        ...
        label_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "models", "MultiLabel", "Labels", f"labels_lead_{lead}.pkl")
    ...
    try:
        for lead in range(12):
            model_path = os.path.join("models\MultiLabel\Models", f"model_lead_{lead}.h5")
            loaded_model = tf.keras.models.load_model(model_path, custom_objects={'AttentionLayer': AttentionLayer})
            models_wavelet[lead] = loaded_model
            label_path = os.path.join("models\MultiLabel\Labels", f"labels_lead_{lead}.pkl")
            with open(label_path, "rb") as f:
                le = pickle.load(f)
            label_encoders_wavelet[lead] = le
...
def select_multi_final_disease(lead_predictions_all, selected_leads, primary_leads=range(0,6), secondary_leads=range(6,12)):
    ...
    label_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "models", "labelMulti.json")
...
def analyze_multi(file_path, selected_leads=None):
    ...
    final_disease , confidence = select_multi_final_disease(lead_predictions_all, selected_leads)
    print(final_disease)
...
if __name__ == "__main__":
 pass
    ...
    # analyze_multi("D:/SETV/Cardiac_Wellness_April9th/Cardiac_Wellness_April8th/Cardiac_Wellness/Cardiac_Flask_April/Flaskapp/uploads/JS08246", selected_leads=[1,2,3,4,7,8,9])
